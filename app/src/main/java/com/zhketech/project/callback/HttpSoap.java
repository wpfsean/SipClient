...
import com.zhketech.project.bean.Device;
...
public class HttpSoap implements Runnable {
	...
	private void createAuthString() {
		...
		mNonce = getNonce();
		mAuthPwd = getPasswordEncode(mNonce, mCamera.getPsw(), mCreated);
	}
	...
	public void run() {
		...
		synchronized (""){
			try {
				initConn(mCamera.getServiceUrl());
				mUrlConn.connect();
				String content = String.format(GET_CAPABILITIES, mCamera.getUserName(),
						mAuthPwd, mNonce, mCreated);
				//	Log.i("tag1","conent:"+content);
				...
				//	Log.i("tag3","mediaUrl:"+mediaUrl);
				initConn(mediaUrl);
				mUrlConn.connect();
				content = String.format(GET_PROFILES, mCamera.getUserName(),
						mAuthPwd, mNonce, mCreated);
				//	Log.i("tag4","content:"+content);
				mUrlConn.getOutputStream().write(content.getBytes());
				inStream = mUrlConn.getInputStream();
				res = inputStream2String(inStream);
				//	Log.i("tag5","res:"+res);
				String profile = getOldProfileToken(res);
				//	Log.i("tag6","profile:"+profile);

				initConn(mediaUrl);
				mUrlConn.connect();
				content = String.format(GET_PROFILE, mCamera.getUserName(), mAuthPwd,
						mNonce, mCreated, profile);
				//	Log.i("tag7","content:"+content);
				mUrlConn.getOutputStream().write(content.getBytes());
				inStream = mUrlConn.getInputStream();
				res = inputStream2String(inStream);
				//	Log.i("tag8","res:"+res);
	...
	private String getURIContent(String profile) {
		String content = String.format(GET_URI_BODY, mCamera.getUserName(),
	...
	public interface OnHttpSoapListener {
		public void OnHttpSoapDone(Device camera, String uri, boolean success);
	}
