	...
	public static void onState(int state, String caller) {
		...
		if (call_state != state) {
			...
			case UserAgent.UA_STATE_INCOMING_CALL:
				Log.i("tag", "1");
				intent.putExtra("msg", "UA_STATE_INCOMING_CALL");
				...
				if ((pstn_state == null || pstn_state.equals("IDLE"))
						&& PreferenceManager
								...
								.getBoolean(
										com.zhketech.sipclient.ui.Settings.PREF_AUTO_ON,
										com.zhketech.sipclient.ui.Settings.DEFAULT_AUTO_ON)
					...
					if (am.getStreamVolume(AudioManager.STREAM_RING) > 0) {
						String sUriSipRingtone = PreferenceManager
								.getDefaultSharedPreferences(mContext)
								.getString(
										com.zhketech.sipclient.ui.Settings.PREF_SIPRINGTONE,
			...
			case UserAgent.UA_STATE_OUTGOING_CALL:
				Log.i("tag", "2");
				intent.putExtra("msg", "UA_STATE_OUTGOING_CALL");
				mContext.sendBroadcast(intent);
				RtpStreamReceiver.good = RtpStreamReceiver.lost = RtpStreamReceiver.loss = RtpStreamReceiver.late = 0;
				RtpStreamReceiver.speakermode = speakermode();
				bluetooth = -1;
			...
			case UserAgent.UA_STATE_IDLE:
				Log.i("tag", "3");
				intent.putExtra("msg", "UA_STATE_IDLE");
				mContext.sendBroadcast(intent);
			...
			case UserAgent.UA_STATE_INCALL:
				Log.i("tag", "4");
				intent.putExtra("msg", "UA_STATE_INCALL");
				mContext.sendBroadcast(intent);
			...
			case UserAgent.UA_STATE_HOLD:
				Log.i("tag", "5");
				intent.putExtra("msg", "UA_STATE_HOLD");
				mContext.sendBroadcast(intent);
				...
				// if (InCallScreen.started && (pstn_state == null ||
				// !pstn_state.equals("RINGING")))
				// mContext.startActivity(createIntent(InCallScreen.class));
				break;
			}
			pos(true);
	...
	public static void onText(int type, String text, int mInCallResId, long base) {
		...
		if (type >= REGISTER_NOTIFICATION
				...
				&& !PreferenceManager
						...
						.getBoolean(
								com.zhketech.sipclient.ui.Settings.PREF_REGISTRATION,
								com.zhketech.sipclient.ui.Settings.DEFAULT_REGISTRATION))
			text = null;
...
//						.getActivity(mContext, 0, createCallLogIntent(), 0));
				if (PreferenceManager.getDefaultSharedPreferences(
						...
						com.zhketech.sipclient.ui.Settings.PREF_NOTIFY,
						com.zhketech.sipclient.ui.Settings.DEFAULT_NOTIFY)) {
				...
				if (base != 0) {
					...
					if (PreferenceManager
							.getDefaultSharedPreferences(mContext)
							.getBoolean(
									com.zhketech.sipclient.ui.Settings.PREF_POS,
									com.zhketech.sipclient.ui.Settings.DEFAULT_POS))
...
//			mNotificationMgr.notify(type, notification);
	...
	static void updateAutoAnswer() {
		if (PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(
				com.zhketech.sipclient.ui.Settings.PREF_AUTO_ONDEMAND,
				com.zhketech.sipclient.ui.Settings.DEFAULT_AUTO_ONDEMAND)
				&& Sipdroid.on(mContext)) {
			if (PreferenceManager
					.getDefaultSharedPreferences(mContext)
					.getBoolean(
							com.zhketech.sipclient.ui.Settings.PREF_AUTO_DEMAND,
							com.zhketech.sipclient.ui.Settings.DEFAULT_AUTO_DEMAND))
				updateAutoAnswer(1);
			else
				updateAutoAnswer(0);
		} else
	...
	public static void pos(boolean enable) {
		...
		if (!PreferenceManager.getDefaultSharedPreferences(mContext)
				.getBoolean(com.zhketech.sipclient.ui.Settings.PREF_POS,
						com.zhketech.sipclient.ui.Settings.DEFAULT_POS)
				|| PreferenceManager
						.getDefaultSharedPreferences(mContext)
						.getString(
								com.zhketech.sipclient.ui.Settings.PREF_POSURL,
								com.zhketech.sipclient.ui.Settings.DEFAULT_POSURL)
						.length() < 1) {
			if (lm != null && am != null) {
				pos_gps(false);
				pos_net(false);
			...
			if (call_state == UserAgent.UA_STATE_IDLE
					...
					&& PreferenceManager
							...
							.getString(
									...
									com.zhketech.sipclient.ui.Settings.DEFAULT_POSURL)
							.length() > 0) {
				@SuppressLint("MissingPermission") Location last = lm
	...
	static void enable_wifi(boolean enable) {
		if (!PreferenceManager.getDefaultSharedPreferences(mContext)
				.getBoolean(com.zhketech.sipclient.ui.Settings.PREF_OWNWIFI,
						com.zhketech.sipclient.ui.Settings.DEFAULT_OWNWIFI))
			return;
		if (enable
				&& !PreferenceManager
						.getDefaultSharedPreferences(mContext)
						.getBoolean(
								com.zhketech.sipclient.ui.Settings.PREF_WIFI_DISABLED,
								com.zhketech.sipclient.ui.Settings.DEFAULT_WIFI_DISABLED))
			return;
		...
		edit.putBoolean(com.zhketech.sipclient.ui.Settings.PREF_WIFI_DISABLED,
				!enable);
		edit.commit();
		/*
	...
	static boolean on_vpn() {
		return PreferenceManager.getDefaultSharedPreferences(mContext)
				.getBoolean(com.zhketech.sipclient.ui.Settings.PREF_ON_VPN,
						com.zhketech.sipclient.ui.Settings.DEFAULT_ON_VPN);
	}
	...
	static void on_vpn(boolean enable) {
		...
		edit.putBoolean(com.zhketech.sipclient.ui.Settings.PREF_ON_VPN, enable);
		edit.commit();
	}
	...
	public static boolean isFast(int i) {
		...
		if (PreferenceManager
				.getDefaultSharedPreferences(mContext)
				.getString(
						com.zhketech.sipclient.ui.Settings.PREF_USERNAME
								+ (i != 0 ? i : ""), "").equals("")
				|| PreferenceManager
						.getDefaultSharedPreferences(mContext)
						.getString(
								com.zhketech.sipclient.ui.Settings.PREF_SERVER
										+ (i != 0 ? i : ""), "").equals(""))
			return false;
		if (wi != null) {
			if (!Sipdroid.release)
			...
			if (wi.getIpAddress() != 0
				...
				if (!on_vpn())
					return PreferenceManager.getDefaultSharedPreferences(
							mContext).getBoolean(
							com.zhketech.sipclient.ui.Settings.PREF_WLAN
									+ (i != 0 ? i : ""),
							com.zhketech.sipclient.ui.Settings.DEFAULT_WLAN);
				else
					return PreferenceManager.getDefaultSharedPreferences(
							mContext).getBoolean(
							com.zhketech.sipclient.ui.Settings.PREF_VPN
									+ (i != 0 ? i : ""),
							com.zhketech.sipclient.ui.Settings.DEFAULT_VPN);
			}
		}
		on_wlan = false;
		return isFastGSM(i);
	...
	static boolean isFastGSM(int i) {
		...
		if (tm.getNetworkType() >= TelephonyManager.NETWORK_TYPE_UMTS)
			return PreferenceManager.getDefaultSharedPreferences(mContext)
					.getBoolean(
							com.zhketech.sipclient.ui.Settings.PREF_3G
									+ (i != 0 ? i : ""),
							com.zhketech.sipclient.ui.Settings.DEFAULT_3G);
		if (tm.getNetworkType() == TelephonyManager.NETWORK_TYPE_EDGE)
			return PreferenceManager.getDefaultSharedPreferences(mContext)
					.getBoolean(
							com.zhketech.sipclient.ui.Settings.PREF_EDGE
									+ (i != 0 ? i : ""),
							com.zhketech.sipclient.ui.Settings.DEFAULT_EDGE);
		return false;
	}
		...
		if (intentAction.equals(Intent.ACTION_BOOT_COMPLETED)) {
			...
			if (!pstn_state.equals("IDLE")
				...
				mHandler.sendEmptyMessageDelayed(MSG_HOLD, 5000);
					...
					&& (call_state == UserAgent.UA_STATE_INCOMING_CALL || call_state == UserAgent.UA_STATE_OUTGOING_CALL))
			...
			if (!PreferenceManager.getDefaultSharedPreferences(mContext)
					.getBoolean(
							com.zhketech.sipclient.ui.Settings.PREF_OWNWIFI,
							com.zhketech.sipclient.ui.Settings.DEFAULT_OWNWIFI))
			...
			if (PreferenceManager
					.getDefaultSharedPreferences(mContext)
					.getBoolean(
							com.zhketech.sipclient.ui.Settings.PREF_SELECTWIFI,
							com.zhketech.sipclient.ui.Settings.DEFAULT_SELECTWIFI))
			...
			if (SystemClock.uptimeMillis() > lastscan + 45000
				...
				if (activeSSID != null
						&& (activeSSID.length() == 0 || activeSSID.equals("0x")))
					activeSSID = null;
				...
				if (configurations != null) {
					...
					if (activescan != null)
						System.out.println("debug wifi asu(active)"
								+ asu(activescan));
					if (bestconfig != null
